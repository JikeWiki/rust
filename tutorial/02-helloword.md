...
- 定义函数: `fn main(){}` 代表一个没有参数没有返回值，名为`main`的函数

- `main`函数: `mian`函数是 Rust 可执行程序的入口

- 打印文本: `println!("Hello, world!")`，Rust 的缩进是4个空格，而不是tab，`println!`是 Rust macro(宏)，如果是函数，就没有`!`，代码以`;`结尾


在以上代码的功能是: 在`mian` 函数中调用 `println!`宏 在控制台打印了一条字符串："Hello, world!"
